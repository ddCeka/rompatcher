...
pub fn compute_psx_hash(path: &Path) -> Result<String, String> {
    ...
    // Read the executable
    let exe_data = read_file_from_iso(&bin_data, &exe_name).or_else(|_| {
        read_file_from_iso(&bin_data, &format!("{};1", exe_name))
    })?;
...
fn read_file_from_iso(
    ...
    // Read root directory
    let root_dir =
        read_sectors(bin_data, root_lba, root_size.div_ceil(DATA_SIZE))?;
    ...
    while offset < root_dir.len() {
        ...
        if name_len > 0 && offset + 33 + name_len <= root_dir.len() {
            ...
            if name_no_version == filename_no_version || name == filename_upper
            {
                ...
                let mut data =
                    read_sectors(bin_data, file_lba, sectors_needed)?;
