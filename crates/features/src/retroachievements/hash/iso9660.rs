...
impl<'a> Iso9660Reader<'a> {
    ...
    fn detect_format(data: &[u8]) -> Result<IsoFormat, String> {
        ...
        if data.len() > std_offset + 6
            && &data[std_offset + 1..std_offset + 6] == b"CD001"
        {
        ...
        if data.len() > raw_offset + 6
            && &data[raw_offset + 1..raw_offset + 6] == b"CD001"
        {
    ...
    fn read_sector(&self, sector: usize) -> Result<&[u8], String> {
        let (offset, size) = match self.format {
            ...
            IsoFormat::Raw => {
                (sector * SECTOR_SIZE_RAW + RAW_DATA_OFFSET, SECTOR_SIZE_ISO)
            }
        };
    ...
    pub fn read_file(&self, filename: &str) -> Result<Vec<u8>, String> {
        ...
        // Read root directory
        let root_dir =
            self.read_sectors(root_lba, root_size.div_ceil(SECTOR_SIZE_ISO))?;
    ...
    pub fn read_file_path(&self, path: &str) -> Result<Vec<u8>, String> {
        ...
        let mut dir_lba = u32::from_le_bytes([
            pvd[156 + 2],
            pvd[156 + 3],
            pvd[156 + 4],
            pvd[156 + 5],
        ]) as usize;
        let mut dir_size = u32::from_le_bytes([
            pvd[156 + 10],
            pvd[156 + 11],
            pvd[156 + 12],
            pvd[156 + 13],
        ...
        for (i, part) in parts.iter().enumerate() {
            let dir_data =
                self.read_sectors(dir_lba, dir_size.div_ceil(SECTOR_SIZE_ISO))?;
            ...
            if i == parts.len() - 1 {
                ...
                let (lba, size) =
                    self.find_directory_in_directory(&dir_data, part)?;
                dir_lba = lba;
                dir_size = size;
            }
        }
    ...
    fn find_file_in_directory(
        ...
        while offset < dir_data.len() {
            ...
            if record_len == 0 {
                let next_sector =
                    ((offset / SECTOR_SIZE_ISO) + 1) * SECTOR_SIZE_ISO;
            ...
            if name_len > 0 && offset + 33 + name_len <= dir_data.len() {
                let name = String::from_utf8_lossy(
                    &dir_data[offset + 33..offset + 33 + name_len],
                )
                .to_uppercase();
                let name_no_version = name.trim_end_matches(";1");

                if name_no_version == filename_no_version
                    || name == filename_upper
                {
                    ...
                    let mut data =
                        self.read_sectors(file_lba, sectors_needed)?;
    ...
    fn find_directory_in_directory(
        ...
        while offset < dir_data.len() {
            ...
            let name_len = dir_data[offset + 32] as usize;
            if name_len > 0
                && offset + 33 + name_len <= dir_data.len()
                && is_dir
            {
                let name = String::from_utf8_lossy(
                    &dir_data[offset + 33..offset + 33 + name_len],
