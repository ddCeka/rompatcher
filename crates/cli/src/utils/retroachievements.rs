...
use rompatcher_features::retroachievements::{
    Console, compute_n64_hash, compute_nds_hash, compute_nes_hash,
    compute_ps2_hash, compute_psp_hash, compute_psx_hash, compute_snes_hash,
    game_url, is_psp_iso, lookup_game_by_hash,
};
use rompatcher_features::validation::algorithms::md5;
use std::path::Path;
...
fn compute_ra_hash(
    ...
    match console {
        ...
        Console::Nds => {
            if rom.len() >= 0x160 {
                let arm9_offset = u32::from_le_bytes([
                    rom[0x20], rom[0x21], rom[0x22], rom[0x23],
                ]);
                let arm9_size = u32::from_le_bytes([
                    rom[0x2C], rom[0x2D], rom[0x2E], rom[0x2F],
                ]);
                let arm7_offset = u32::from_le_bytes([
                    rom[0x30], rom[0x31], rom[0x32], rom[0x33],
                ]);
                let arm7_size = u32::from_le_bytes([
                    rom[0x3C], rom[0x3D], rom[0x3E], rom[0x3F],
                ]);
                let icon_offset = u32::from_le_bytes([
                    rom[0x68], rom[0x69], rom[0x6A], rom[0x6B],
                ]);
                ...
                trace!(
                    "Icon/Title: offset=0x{:X}, size=0xA00 bytes",
                    icon_offset
                );
            }
