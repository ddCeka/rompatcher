...
use rompatcher_cli::OnlyMode as OnlyModeLib;
...
fn main() -> Result<()> {
    ...
    let only_modes: Vec<OnlyModeLib> =
        cli.only.into_iter().map(|m| m.into()).collect();

    // Validate: patch is required unless --only ra
    if cli.patch.is_none()
        && !only_modes.iter().any(|m| matches!(m, OnlyModeLib::Ra))
    {
        anyhow::bail!("Patch file is required (unless using --only ra)");
    }

    commands::apply::execute(
        cli.rom, cli.patch, cli.output, cli.verify, only_modes,
    )
}
