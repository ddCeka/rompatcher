...
use rompatcher_formats::detect_format;
...
pub fn execute(
    ...
    verify: bool,
    only_modes: Vec<rompatcher_cli::OnlyMode>,
) -> Result<()> {
    ...
    if !only_modes.is_empty() {
        for mode in &only_modes {
            match mode {
                rompatcher_cli::OnlyMode::Ra => {
                    only::handle_ra_mode(&rom_path)?;
                }
                rompatcher_cli::OnlyMode::Verify => {
        ...
        if !only_modes
            .iter()
            .any(|m| matches!(m, rompatcher_cli::OnlyMode::Verify))
        {
            return Ok(());
        }
    }
    ...
    let patch_path =
        patch_path.expect("Patch path should be validated in main");
    ...
    let patch_type = detect_format(&patch_data)
        .context("Could not detect patch format from file header")?;
    ...
    if only_modes
        ...
        .any(|m| matches!(m, rompatcher_cli::OnlyMode::Verify))
    {
        return only::handle_verify_mode(
            &original_rom,
            &patch_data,
            &patch_type,
        );
    ...
    if verify {
        ...
        if patch_type.name() == "International Patching System" {
            warn!(
                "IPS format does not support checksum verification (no embedded checksums)"
            );
        } else {
            info!("Verifying source ROM checksum...");
            super::verify::verify_source(
                &original_rom,
                &patch_data,
                &patch_type,
            )
