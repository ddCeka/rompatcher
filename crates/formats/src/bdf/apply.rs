...
pub fn apply_patch(rom: &mut Vec<u8>, patch: &[u8]) -> Result<()> {
    ...
    if patched_size > crate::bdf::constants::MAX_PATCHED_SIZE {
        return Err(rompatcher_core::PatchError::InvalidFormat(format!(
    ...
    let control_compressed_data = &patch[cursor.position() as usize
        ..(cursor.position() as usize + control_size)];
    cursor.seek(SeekFrom::Current(control_size as i64))?;

    let diff_compressed_data = &patch
        [cursor.position() as usize..(cursor.position() as usize + diff_size)];
