...
use crate::xdelta::{
    ...
    constants::{
        VCD_ADD, VCD_APPHEADER, VCD_CODETABLE, VCD_COPY, VCD_DECOMPRESS,
        VCD_NOOP, VCD_RUN, VCD_SOURCE, VCD_TARGET,
    },
    headers::{WindowHeader, calculate_target_size},
    parser::VcdiffParser,
};
use rompatcher_core::{PatchError, Result};
use rompatcher_features::validation::algorithms::adler32;
use std::io::Read;
...
pub fn apply_patch(rom: &mut Vec<u8>, patch: &[u8]) -> Result<()> {
    ...
    while !parser.is_eof() {
        ...
        let instructions_offset =
            add_run_data_offset + win_header.add_run_data_length;
        let addresses_offset =
            instructions_offset + win_header.instructions_length;

        let mut add_run_stream =
            VcdiffParser::new_at_offset(patch, add_run_data_offset);
        let mut inst_stream =
            VcdiffParser::new_at_offset(patch, instructions_offset);
        let mut addr_stream =
            VcdiffParser::new_at_offset(patch, addresses_offset);
        ...
        while inst_stream.position() < addresses_offset {
            ...
            for instruction in &code_table[instruction_index] {
                ...
                match instruction.inst_type {
                    ...
                    VCD_COPY => {
                        let here_val =
                            add_run_data_index + win_header.source_length;
                        ...
                        if addr < win_header.source_length {
                            ...
                            abs_addr = target_window_position
                                + (addr - win_header.source_length);
                            use_source = false;
                        }
