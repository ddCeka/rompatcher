...
fn calculate_max_offset(
    ...
    while offset + 3 <= patch.len() {
        ...
        if record_offset == EOF_MARKER {
            if let Some(truncate_size) =
                try_read_truncate_size(patch, offset + 3)
            {
        ...
        let (new_offset, record_end) =
            process_record(patch, offset, record_offset as usize)?;
