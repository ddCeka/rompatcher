...
use super::varint;
use rompatcher_core::{PatchError, PatchMetadata, PatchType, Result};
...
pub fn extract_metadata(patch: &[u8]) -> Result<PatchMetadata> {
    ...
    // Parse source size
    let (source_size, bytes_read) =
        varint::decode(&patch[offset..]).map_err(|_| {
            PatchError::InvalidFormat("Invalid source size varint".to_string())
        })?;
    offset += bytes_read;
    metadata.source_size = Some(source_size as usize);

    // Parse target size
    let (target_size, bytes_read) =
        varint::decode(&patch[offset..]).map_err(|_| {
            PatchError::InvalidFormat("Invalid target size varint".to_string())
        })?;
    offset += bytes_read;
    ...
    let (metadata_size, bytes_read) = varint::decode(&patch[offset..])
        .map_err(|_| {
            PatchError::InvalidFormat(
                "Invalid metadata size varint".to_string(),
            )
        })?;
    offset += bytes_read;
    ...
    if metadata_size > 0 {
        let metadata_end =
            offset.checked_add(metadata_size as usize).ok_or_else(|| {
                PatchError::InvalidFormat("Metadata size too large".to_string())
            })?;
        ...
        if let Ok(metadata_str) =
            String::from_utf8(patch[offset..metadata_end].to_vec())
        {
            metadata =
                metadata.with_extra("metadata".to_string(), metadata_str);
        }
    }
