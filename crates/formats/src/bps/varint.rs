...
pub fn decode(data: &[u8]) -> Result<(u64, usize)> {
    ...
    for &byte in data.iter() {
        ...
        let part =
            ((byte & 0x7f) as u64).checked_mul(shift).ok_or_else(|| {
                PatchError::Other("Varint multiplication overflow".to_string())
            })?;
        ...
        shift = shift.checked_shl(7).ok_or_else(|| {
            PatchError::Other("Varint shift overflow".to_string())
        })?;

        result = result.checked_add(shift).ok_or_else(|| {
            PatchError::Other("Varint addition overflow".to_string())
        })?;
