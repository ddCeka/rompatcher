...
pub fn parse_header(patch: &[u8]) -> Result<(u64, u64, usize)> {
    ...
    let (source_size, bytes_read) =
        varint::decode(&patch[offset..]).map_err(|_| {
            PatchError::InvalidFormat("Invalid source size".to_string())
        })?;
    offset += bytes_read;

    let (target_size, bytes_read) =
        varint::decode(&patch[offset..]).map_err(|_| {
            PatchError::InvalidFormat("Invalid target size".to_string())
        })?;
    offset += bytes_read;

    let (metadata_size, bytes_read) = varint::decode(&patch[offset..])
        .map_err(|_| {
            PatchError::InvalidFormat("Invalid metadata size".to_string())
        })?;
