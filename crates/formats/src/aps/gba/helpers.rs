...
pub fn parse_header(patch: &[u8]) -> Result<(GbaHeader, usize)> {
    ...
    let source_size =
        u32::from_le_bytes([patch[4], patch[5], patch[6], patch[7]]);
    let target_size =
        u32::from_le_bytes([patch[8], patch[9], patch[10], patch[11]]);
...
pub fn parse_record(patch: &[u8], offset: usize) -> Result<GbaRecord> {
    ...
    let source_crc16 =
        u16::from_le_bytes([patch[offset + 4], patch[offset + 5]]);

    let target_crc16 =
        u16::from_le_bytes([patch[offset + 6], patch[offset + 7]]);
