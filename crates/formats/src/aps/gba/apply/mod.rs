...
pub fn apply(rom: &[u8], patch: &[u8]) -> Result<Vec<u8>> {
    ...
    output
        ...
        .map_err(|_| {
            PatchError::Other(
                "Failed to allocate memory for target ROM".to_string(),
            )
        })?;
...
pub fn verify(rom: &[u8], patch: &[u8]) -> Result<()> {
    ...
    while offset < patch.len() {
        ...
        let block =
            &rom[record.offset as usize..record.offset as usize + BLOCK_SIZE];
        let calculated_crc =
            crc16::State::<crc16::CCITT_FALSE>::calculate(block);
