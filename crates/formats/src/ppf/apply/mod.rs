...
pub fn apply_patch(rom: &mut [u8], patch: &[u8]) -> Result<()> {
    ...
    while patch_cursor.position() < patch.len() as u64 {
        ...
        if (patch.len() as u64 - current_pos) >= 4 {
            ...
            if patch_cursor.read_exact(&mut magic_check).is_ok()
        ...
        // Apply to ROM
        let end_offset =
            (offset as usize).checked_add(data_len).ok_or_else(|| {
                PatchError::Other("Offset + data_len overflow".to_string())
            })?;
