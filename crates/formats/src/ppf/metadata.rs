...
pub fn extract_metadata(patch: &[u8]) -> Result<PatchMetadata> {
    ...
    metadata = metadata
        .with_extra("version".to_string(), format!("PPF{}", header.version));

    if !header.description.is_empty() {
        metadata =
            metadata.with_extra("description".to_string(), header.description);
    }
    ...
    if header.version == 3 {
        metadata = metadata
            ...
            .with_extra(
                "block_check".to_string(),
                header.block_check.to_string(),
            )
        ...
        metadata =
            metadata.with_extra("block_check".to_string(), "true".to_string());
    }
    ...
    while cursor.position() < patch.len() as u64 {
        ...
        if (patch.len() as u64 - current_pos) >= 4 {
            ...
            if cursor.read_exact(&mut magic_check).is_ok()
                && magic_check == FILE_ID_DIZ_MAGIC
            {
                // Found FILE_ID.DIZ
