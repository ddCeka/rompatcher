...
impl RupMetadata {
    ...
    pub fn from_patch(patch: &[u8]) -> Self {
        Self {
            text_encoding: patch
                .get(OFFSET_TEXT_ENCODING)
                .copied()
                .unwrap_or(0),
            author: helpers::parse_metadata_string(
                patch,
                OFFSET_AUTHOR,
                SIZE_AUTHOR,
            ),
            version: helpers::parse_metadata_string(
                patch,
                OFFSET_VERSION,
                SIZE_VERSION,
            ),
            title: helpers::parse_metadata_string(
                patch,
                OFFSET_TITLE,
                SIZE_TITLE,
            ),
            genre: helpers::parse_metadata_string(
                patch,
                OFFSET_GENRE,
                SIZE_GENRE,
            ),
            language: helpers::parse_metadata_string(
                patch,
                OFFSET_LANGUAGE,
                SIZE_LANGUAGE,
            ),
