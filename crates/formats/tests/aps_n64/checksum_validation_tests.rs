...
use rompatcher_formats::aps::n64::ApsN64Patcher;
use std::fs;
...
fn test_zelda_oot_spanish_patch() {
    ...
    let mut rom = match fs::read(&rom_path) {
        ...
        Err(_) => {
            ...
            println!(
                "Download: The Legend of Zelda: Ocarina of Time (USA).z64"
            );
            return;
        }
    };
    ...
    let metadata =
        ApsN64Patcher::metadata(&patch).expect("Failed to extract metadata");
...
fn test_rom_file_exists() {
    ...
    if let Ok(rom) = fs::read(&rom_path) {
        ...
        println!(
            "This is expected if you haven't downloaded the base ROM yet."
        );
    }
}
