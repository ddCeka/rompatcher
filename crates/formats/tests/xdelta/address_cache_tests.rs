...
use rompatcher_formats::xdelta::address_cache::AddressCache;
use rompatcher_formats::xdelta::{
...
fn test_decode_address_self_mode() {
    ...
    let addr =
        decode_address(&mut cache, &mut parser, 100, VCD_MODE_SELF).unwrap();
    assert_eq!(addr, 5);
}
...
fn test_decode_address_here_mode() {
    ...
    let addr =
        decode_address(&mut cache, &mut parser, 100, VCD_MODE_HERE).unwrap();
    assert_eq!(addr, 95);
}
