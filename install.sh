...
# Copy binary
cp "$BINARY_PATH" "$INSTALL_DIR/rompatcher"

echo -e "${COLOR_SUCCESS}==> Installed 'rompatcher' to $INSTALL_DIR/rompatcher${NC}"
...
if ! echo ":$PATH:" | grep -q ":$INSTALL_DIR:"; then
    ...
    echo -e "${COLOR_SUCCESS}==> Installation complete! Run 'rompatcher --help' to get started.${NC}"
fi