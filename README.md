...
**rompatcher** is a fast, safe ROM patcher written in Rust. It applies patches to ROM files with automatic format detection and optional checksum verification.
...
```bash
git clone https://github.com/ddCeka/rompatcher.git
cd rompatcher
...
rompatcher game.gb patch.ips

# With output path
rompatcher game.gb patch.ips patched.gb

# With checksum verification
rompatcher game.gba patch.bps --verify

# Check ROM against RetroAchievements
rompatcher game.snes --only ra
```
...
rompatcher game.gbc patch.bps --only verify

# Check RetroAchievements hash
rompatcher game.sfc --only ra

# Both
rompatcher game.gbc patch.bps --only verify ra
```

### Verbosity

```bash
rompatcher game.gb patch.ips -v      # Verbose output
rompatcher game.gb patch.ips -vv     # Extra verbose
rompatcher game.gb patch.ips -q      # Quiet (errors only)
```
